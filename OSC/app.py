...
def return_file():
    ...
    return _download(filename, list_directory[0])
...
def _download(path: str, name: str):
    kwargs = dict(as_attachment=True)
    try:
        # Flask >= 2.0
        return send_file(path, download_name=name, **kwargs)
    except TypeError:
        # Flask 1.1.x
        return send_file(path, attachment_filename=name, **kwargs)
